...
export default function App() {
  ...
  // Đẩy params xuống camera mỗi khi thay đổi (camera dùng method, không phải prop)
  useEffect(() => {
    if (mode !== 'camera') return;
    cameraRef.current?.setBeautyParams({
      smoothing: params.smoothing,
      whitening: params.whitening,
    });
    cameraRef.current?.setReshapeParams({
      faceSlim: params.faceSlim,
      eyeEnlarge: params.eyeEnlarge,
    });
    cameraRef.current?.setMakeupParams({ blusher: params.blusher });
  }, [params, mode]);
    ...
    <SafeAreaView style={styles.root}>
      ...
      <View style={styles.preview}>
        {mode === 'image' ? (
          ...
          <GPUPixelCameraView ref={cameraRef} style={styles.fill} />
        )}
      </View>
