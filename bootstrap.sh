...
[[ -d "${ROOT}/prebuilt-sdk/ios/BeautyFilter.xcframework" ]] || {
  echo "Không thấy prebuilt-sdk/ios/BeautyFilter.xcframework — thiếu SDK dựng sẵn (xem README)."
  exit 1
}

# ---- 1. Sinh project RN (nếu chưa có) ---------------------------------------
if [[ -d "${APP}" ]]; then
  echo "${APP_NAME}/ đã tồn tại — bỏ qua bước init, chỉ cập nhật overlay."
else
