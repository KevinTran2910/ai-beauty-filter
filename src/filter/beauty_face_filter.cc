...
bool BeautyFaceFilter::Init() {
  ...
  bilateral_filter_ = BilateralFilter::Create();
  AddFilter(bilateral_filter_);
  bilateral_filter_->setDistanceNormalizationFactor(8.0f);
  ...
  bilateral_filter_->AddSink(beauty_face_filter_, 1);
  box_high_pass_filter_->AddSink(beauty_face_filter_, 2);

  SetTerminalFilter(beauty_face_filter_);

  bilateral_filter_->SetTexelSpacingMultiplier(4.0f);
  SetRadius(4);
...
void BeautyFaceFilter::SetRadius(float radius) {
  bilateral_filter_->setDistanceNormalizationFactor(radius * 2.0f);
