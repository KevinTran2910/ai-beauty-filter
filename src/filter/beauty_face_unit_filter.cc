...
const std::string kGPUImageBaseBeautyFaceFragmentShaderString = R"(
    ...
    // Edge detection kernel for better edge preservation
    vec3 detectEdges(vec2 texCoord) {
      vec3 center = texture2D(inputImageTexture, texCoord).rgb;
      vec3 top = texture2D(inputImageTexture, texCoord + vec2(0.0, -0.003)).rgb;
      vec3 bottom = texture2D(inputImageTexture, texCoord + vec2(0.0, 0.003)).rgb;
      vec3 left = texture2D(inputImageTexture, texCoord + vec2(-0.003, 0.0)).rgb;
      vec3 right = texture2D(inputImageTexture, texCoord + vec2(0.003, 0.0)).rgb;
      
      // Sobel-like edge detection
      vec3 dx = (right - left) * 0.5;
      vec3 dy = (bottom - top) * 0.5;
    ...
    void main() {
      ...
      if (blurAlpha >= 0.0) {
        // Calculate edge mask to preserve details
        vec3 edgeMask = detectEdges(textureCoordinate);
        float edgeIntensity = (edgeMask.r + edgeMask.g + edgeMask.b) / 3.0;
        
        // Adjust skin detection parameter
        float theta = 0.1;
        float p = clamp((min(iColor.r, meanColor.r - 0.1) - 0.2) * 4.0, 0.0, 1.0);
        float meanVar = (varColor.r + varColor.g + varColor.b) / 3.0;
        
        // Improved blending factor with edge preservation
        float kMin;
        highp vec3 resultColor;
        
        // Reduce blurring effect at edges
        float edgePreserveFactor = 1.0 - clamp(edgeIntensity * 5.0, 0.0, 1.0);
        
        // Combine skin detection with edge preservation
        kMin = (1.0 - meanVar / (meanVar + theta)) * p * blurAlpha * edgePreserveFactor;
        kMin = clamp(kMin, 0.0, 1.0);
        
        // Blend original and blurred image
        resultColor = mix(iColor.rgb, meanColor.rgb, kMin);

        // Improved high-pass filtering for detail preservation
        ...
        // Add details back based on edge detection
        float detailRecovery = sharpen * (1.0 + edgeIntensity);
        color = resultColor + detailRecovery * hPass * 2.0;
      }
