...
// Add mutex for thread safety in WebAssembly environment
...
CustomFaceDetector::CustomFaceDetector(int num_threads): num_threads_(num_threads) {
  ...
  if (detector_) {
    ...
    // For WebAssembly, limit thread usage to avoid issues
    #ifdef __EMSCRIPTEN__
    detector_->setNumThreads(1);  // Use single thread in WebAssembly
    #else
...
void CustomFaceDetector::SetNumThreads(int num_threads) {
  if (detector_) {
    #ifdef __EMSCRIPTEN__
    // Ignore thread count changes in WebAssembly
    #else
...
std::vector<float> CustomFaceDetector::Detect(const uint8_t* data, int width, int height, 
  ...
  // Add additional landmarks only if we have enough base landmarks
  if (normalized_landmarks.size() >= 206) { // 103 landmarks × 2 coordinates
